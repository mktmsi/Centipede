...
double Bstan = 2.0, Bswin = 0010.0, alpha = 1.0;
...
Vector2D TangentVector, NormalVector; //接戦ベクトル，法線ベクトル
...
const double mu = (0.5)*2.0;
const double c_tanh = 10.0;

const double xstart = 20.0, xend =xstart+ 100.0;
const double ystart=0.0,a=0.2;
double yend=ystart+(xend-xstart)*a,b=ystart-a*xstart;
...
void init()
{
  ...
  NormalVector.set_vec(0.0, 1.0);
  TangentVector.set_x(NormalVector.get_y() * sin(-PI / 2)); //回転行列
  TangentVector.set_y(NormalVector.get_x() * sin(PI / 2));  //回転行列
...
void func(Mass *pt, Mass *pt_out, Mass *pl, Mass *pl_out, double *pPhi, double *pPhi_out)
{
  ...
  /***********************************************/
  //**********各質点にかかる他の質点からのバネ・ダンパ由来の力ベクトルを更新********/
  /***********************************************/
  for (i = 0; i < N; i++)
  {
    if (i == 0) //あってる
  ...
  double xabs = 0.0, tanX = 0.0, temp = 0.0;
  /***********************************************/
  //************床からの粘弾性反力と摩擦力********///////
  /***********************************************/
  for (i = 0; i < N - 1; i++)
  {
    /********:体節*********/
    if (xstart <= trunk[i].aa.r.get_x() && trunk[i].aa.r.get_x() <= xend) //坂道にいるとき
    {
      NormalVector.set_vec(-a, 1.0);
      temp = a * trunk[i].aa.r.get_x() + b; //坂道の高さ
      if (trunk[i].aa.r.get_y() < temp)     //接地したとき
      {
        temp = trunk[i].aa.r.get_y() - temp;                //バネの縮み，負の値になるようにする
        temp = -k_gr * temp - c_gr * trunk[i].aa.v.get_y(); //力
        temp = max(temp, 0.0);
        F_lGround[i] = NormalVector * temp;
        //F_lGround[i].set_x(0.0);
        //F_lGround[i].set_y(max(-k_gr * trunk[i].aa.r.get_y() - c_gr * trunk[i].aa.v.get_y(), 0.0));
      }
      else
      {
    ...
    else //平地にいるとき
    {
      ...
      if (trunk[i].aa.r.get_y() < 0.0) //接地したとき
      {
        temp = max(-k_gr * trunk[i].aa.r.get_y() - c_gr * trunk[i].aa.v.get_y(), 0.0);
        F_lGround[i] = NormalVector * temp;
        //F_lGround[i].set_x(0.0);
        //F_lGround[i].set_y(max(-k_gr * trunk[i].aa.r.get_y() - c_gr * trunk[i].aa.v.get_y(), 0.0));
      }
    ...
    F_tFriction[i] = TangentVector * (-mu * xabs * tanX);
    //F_tFriction[i].set_vec(-mu * xabs * tanX, 0.0);

    /********脚************/
    if (xstart <= leg[i].aa.r.get_x() && leg[i].aa.r.get_x() <= xend) //坂道にいるとき
    {
      NormalVector.set_vec(-a, 1.0);

      temp = a * leg[i].aa.r.get_x() + b; //坂道の高さ
      if (leg[i].aa.r.get_y() < temp)     //接地したとき
      {
        temp = leg[i].aa.r.get_y() - temp; //バネの縮み，負の値になるようにする
        //temp=-k_gr*temp- c_gr * leg[i].aa.v.get_y();//力　間違ってる
        temp = -k_gr * temp - c_gr * (leg[i].aa.v * NormalVector); //力　
    ...
    else //平地にいるとき
    {
      ...
      if (leg[i].aa.r.get_y() < 0.0) //接地したとき
      {
        temp = max(-k_gr * leg[i].aa.r.get_y() - c_gr * leg[i].aa.v.get_y(), 0.0);
        F_lGround[i] = NormalVector * temp;
        //F_lGround[i].set_x(0.0);
        //F_lGround[i].set_y(max(-k_gr * leg[i].aa.r.get_y() - c_gr * leg[i].aa.v.get_y(), 0.0));
      }
    ...
    xabs = fabs(F_lGround[i].get_abs());
    tanX = tanh(c_tanh * leg[i].aa.v.get_x());
    F_lFriction[i] = TangentVector * (-mu * xabs * tanX);
...
void display(void)
{
  ...
  //地面
  monitor.SetAllColor(0.0, 0.0, 0.0);
  monitor.DrawLine(0.3 * -50, 0.0, xstart / L * 2 - 1.5, 0.0, 0.5);
  monitor.DrawLine(xstart / L * 2 - 1.5, 0.0, xend / L * 2 - 1.5, yend / L * 2, 0.5);
  //縦線
  for (j = -N / 3; j < 300; j++)
  {
...
/*
  ...
  F_x = base_x + F.get_x()/ L;
  F_y = base_y + F.get_y()/ L;
  monitor.SetAllColor(0.0, 0.0, 0.0);
